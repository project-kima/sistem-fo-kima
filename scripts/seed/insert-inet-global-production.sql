-- ============================================================================
-- UPSERT DATA CUSTOMER BERHENTI PT INET GLOBAL - PRODUCTION
-- ============================================================================
-- Tanggal: 2026-05-17
...
-- - PT Nippon Indosari Corpindo Tbk 1/8 dimasukkan sebagai sharing_core.
-- - Maintenance dimasukkan sebagai customer terpisah dengan core_total 1 karena data sumber tidak memiliki rasio sharing.
-- - BAK '-' atau kosong memakai nomor teknis NO-BAK-*.
-- - Status invoice kosong/'-' dimasukkan sebagai 'belum_ditagih'.
-- ============================================================================
...
BEGIN
  ...
  FROM isps
  WHERE lower(trim(name)) = lower(trim('PT Inet Global'))
     OR lower(name) LIKE '%inet global%'
  ORDER BY CASE
    WHEN lower(trim(name)) = lower(trim('PT Inet Global')) THEN 1
    WHEN lower(name) LIKE '%inet global%' THEN 2
    ELSE 3
  END, id
  LIMIT 1;
  ...
  IF v_isp_id IS NULL THEN
    ...
    VALUES ('PT Inet Global', 'aktif', 'shared', 0, 'monthly', 0, NOW(), NOW())
  ...
  FOR row_data IN
    SELECT *
    FROM (VALUES
      ('CUST-INET-NIPPON-INDOSARI-CORPINDO-STOPPED', 'PT Nippon Indosari Corpindo Tbk', DATE '2023-06-23', DATE '2023-06-23', DATE '2024-06-22', 'sharing_core', 0, '1/8', 'KIMA.BAK-28/DBO/FO/VI/2023', '128/INV.FO/VI/2023', 'lunas', 500000::numeric, 6000000::numeric, NULL::numeric, 12, 'Berhenti'),
      ('CUST-INET-NIPPON-INDOSARI-CORPINDO-STOPPED', 'PT Nippon Indosari Corpindo Tbk', DATE '2023-06-23', DATE '2024-06-23', DATE '2025-06-22', 'sharing_core', 0, '1/8', 'KIMA.BAK-27/DBO/FO/VI/2024', 'INV-033/KIMA/FO/VI/2024', 'lunas', 500000::numeric, 6000000::numeric, NULL::numeric, 12, 'Berhenti'),
      ('CUST-INET-MAINTENANCE-STOPPED', 'Maintenance', DATE '2023-10-01', DATE '2023-10-01', DATE '2025-06-30', 'core', 1, NULL, '-', 'INET-MAINTENANCE', 'belum_ditagih', 1000000::numeric, 21000000::numeric, NULL::numeric, 21, 'Berhenti')
  ...
  LOOP
    ...
    IF v_customer_id IS NULL THEN
      ...
      VALUES (row_data.customer_code, 'PT Inet Global', row_data.customer_name, 'berhenti', COALESCE(row_data.activation_fee_amount, 0), row_data.cooperation_start_date, NOW(), NOW())
      RETURNING id INTO v_customer_id;
    ELSE
      UPDATE customers
      SET
        isp_name = 'PT Inet Global',
...
WHERE c.customer_code IN (
  'CUST-INET-NIPPON-INDOSARI-CORPINDO-STOPPED',
  'CUST-INET-MAINTENANCE-STOPPED'
)
