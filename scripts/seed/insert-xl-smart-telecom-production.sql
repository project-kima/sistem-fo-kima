-- ============================================================================
-- UPSERT DATA CUSTOMER PT XLSMART TELECOM SEJAHTERA TBK / XL AXIATA - PRODUCTION
-- ============================================================================
-- Tanggal: 2026-05-17
...
-- ============================================================================
-- Catatan import:
-- - ISP mengikuti spreadsheet: PT XL Axiata Tbk.
-- - Customer: PT Xlsmart Telecom Sejahtera Tbk.
-- - Alokasi 2 core, periode 3 bulan, invoice lunas.
-- ============================================================================
  ...
  FOR row_data IN
    ...
    FROM (VALUES
      (
        ...
        2,
        NULL,
        'KIMA.BAK-31/DBO/FO/VII/2025 dan 107/FOIN-XLSMART/X/2025',
        '085/FO/10/25',
        'lunas',
        NULL,
        3000000::numeric,
    ...
    ) AS value(isp_name, customer_code, customer_name, cooperation_start_date, contract_start_date, contract_end_date, core_type, core_total, sharing_ratio, contract_number, invoice_seed, invoice_status, remarks, monthly_amount, yearly_amount, activation_fee_amount, invoice_count)
  LOOP
    ...
    IF v_contract_id IS NULL THEN
      ...
      VALUES (v_customer_id, v_contract_number, row_data.contract_start_date, row_data.contract_end_date, row_data.core_type::core_allocation_type, COALESCE(row_data.core_total, 0), row_data.sharing_ratio, (CASE WHEN row_data.contract_end_date >= CURRENT_DATE THEN 'aktif' ELSE 'expired' END)::contract_status, 1, 'bulan', NOW(), NOW())
      RETURNING id INTO v_contract_id;
    ELSE
      UPDATE contracts
      SET start_date = row_data.contract_start_date,
    ...
    IF v_version_id IS NULL THEN
      ...
      VALUES (
        ...
        COALESCE(row_data.remarks, 'Imported from XL Smart spreadsheet batch'),
        NOW(),
        NOW()
      )
...
WHERE c.customer_code = 'CUST-XL-SMART-TELECOM-CORE-2'
