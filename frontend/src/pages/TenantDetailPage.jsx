...
function TenantDetailPage({ customer, contextIsp, initialTab = "overview", onBack, onEditTenant, onNavigate, onRefreshAll }) {
    ...
    const routePoints = Array.isArray(detail?.route?.points) ? detail.route.points : [];
    const routeVersions = Array.isArray(detail?.route?.versions) ? detail.route.versions : [];
    const routeHistory = Array.isArray(detail?.route?.history) ? detail.route.history : [];
    const [expandedHistoryIds, setExpandedHistoryIds] = useState([]); // Default closed as requested

    const activeAwalPoint = routePoints.find((point) => point.pointType === "awal") ?? null;
    const activeTujuanPoint = routePoints.find((point) => point.pointType === "tujuan") ?? null;
    ...
    const getInvoiceFollowUps = (invoice) => {
        const followUps = Array.isArray(invoice?.invoiceFollowUps) ? invoice.invoiceFollowUps : [];
        return [...followUps].sort((left, right) => Number(left?.splitOrder ?? 0) - Number(right?.splitOrder ?? 0));
    };

    const hasUploadedInvoiceSplit = (invoice) => getInvoiceFollowUps(invoice)
        .some((followUp) => isOpenableFileUrl(followUp?.invoiceFileUrl));
            ...
            invoiceRows.forEach((invoice) => {
                ...
                getInvoiceFollowUps(invoice).forEach((followUp) => {
                    ...
                    nextFollowUpDrafts[draftKey] = {
                        invoiceNumber: String(previousFollowUpDrafts[draftKey]?.invoiceNumber ?? followUp?.invoiceNumber ?? ""),
                    };
                });

                nextFollowUpDrafts.initial = {
                    invoiceNumber: String(previousFollowUpDrafts.initial?.invoiceNumber ?? ""),
                };
    ...
    const workflowInvoiceRows = useMemo(
        () => invoiceRows.map((invoice) => {
            ...
            const activeReminderSplits = getInvoiceFollowUps(invoice)
                .filter((followUp) => followUp?.status !== "completed" && !isOpenableFileUrl(followUp?.invoiceFileUrl));
    ...
    const unpaidInvoiceCount = invoiceRows.filter((invoice) => ["unpaid", "overdue"].includes(invoice.statusMeta.key)).length;
    const pendingInvoiceCount = invoiceRows.filter((invoice) => invoice.statusMeta.key === "pending").length;
    const activeInvoiceReminderCount = workflowInvoiceRows.reduce(
        (total, invoice) => total + Number(invoice.activeReminderSplits?.length ?? 0),
        0,
    );
    ...
    const nextActionMeta = (() => {
        ...
        return {
            ...
            message: "Mendekati jatuh tempo. Isi nomor invoice awal lalu upload invoice untuk pembayaran ini.",
            dueDate,
        };
    })();

    const primaryContractRowMarkerId = versions.length > 0
        ? `version-${versions[0]?.id ?? 0}`
        : (contract ? `contract-${contract.id}` : null);
    const hasContractNumberValue = Boolean(String(contract?.contractNumber ?? "").trim());
    ...
    if (contract && !hasContractNumberValue && !isContractNumberExplicitlyEmpty) {
        derivedNeedActionTodos.push({
            ...
            message: "Isi nomor kontrak lokasi atau tandai memang kosong jika datanya memang tidak ada.",
            dueDate: null,
        });
    }
    ...
    const contractRowsForTable = versions.length > 0
        ? versions.map((version, index) => {
            const normalizedCoreType = String(version?.coreType ?? "").toLowerCase();
            ...
            return {
                ...
                renewalFollowUps: Array.isArray(version?.renewalFollowUps) ? version.renewalFollowUps : [],
            };
        })
        : (contract
            ? [{
                ...
                paket: String(detail?.paket || customer?.paket || "CORE").toUpperCase(),
    ...
    const openBillingEditor = () => {
        if (!contract?.id) {
            setError("Kontrak beroperasi tidak ditemukan untuk update periode tagihan.");
            return;
        }
    ...
    const handleSaveBillingCycle = async (event) => {
        ...
        if (!["hari", "bulan", "tahun"].includes(String(billingEditor.billingUnit))) {
        ...
        try {
            await fetchJson(`${API_BASE_URL}/api/customers/${customer.id}/contracts/${contract.id}`, {
                method: "PATCH",
            ...
            setInvoiceFeedback("Periode tagihan berhasil diperbarui. Jadwal invoice aktif direstrukturisasi, sementara invoice lunas tetap tersimpan sebagai riwayat.");
            await Promise.all([loadDetail(), onRefreshAll?.()]);
        } catch (requestError) {
            setBillingError(requestError instanceof Error ? requestError.message : "Gagal memperbarui periode tagihan.");
        } finally {
    ...
    const handleSaveContractNumber = async () => {
        if (!contract?.id) {
            setError("Kontrak beroperasi tidak ditemukan untuk update nomor kontrak.");
            return;
        }
        ...
        try {
            ...
            setError(requestError instanceof Error ? requestError.message : "Gagal menyimpan nomor kontrak.");
        } finally {
    ...
    const handleCreateVersion = async (event) => {
        ...
        if ((versionEditor.reason ?? "ubah_paket") === "lainnya" && !String(versionEditor.customReason ?? "").trim()) {
    ...
    const handleUploadDocument = async (event) => {
        ...
        try {
            ...
            if (documentDraft.nomorDokumen.trim()) formData.append("nomorDokumen", documentDraft.nomorDokumen.trim());
            if (documentDraft.tanggalDokumen) formData.append("tanggalDokumen", documentDraft.tanggalDokumen);
            if (contract?.id) formData.append("contractId", String(contract.id));
            if (documentDraft.contractVersionId) formData.append("contractVersionId", String(Number(documentDraft.contractVersionId)));
        ...
        () =>
            routeHistory.map((item) => {
                ...
                return {
                    ...
                    operationLabel: routeOperationLabelMap[item?.operation] ?? toTitleCase(item?.operation ?? "perubahan"),
    ...
    const renderRouteSnapshotPills = (points, emptyLabel) => {
        if (!Array.isArray(points) || points.length === 0) {
            return <span className="text-[10px] italic text-slate-400 font-medium">{emptyLabel}</span>;
        }

        return (
            <div className="relative pl-3">
                {/* Vertical Line Connector */}
                <div className="absolute left-1.5 top-2 bottom-2 w-0.5 bg-gradient-to-b from-slate-200 via-slate-100 to-transparent" />

                <div className="space-y-3">
                    {points.map((point, index) => {
                        const pointMeta = routePointTypeMeta[point?.pointType] ?? routePointTypeMeta.transit;
                        const toneClass = point?.pointType === "awal"
                            ? "bg-emerald-500 ring-emerald-100 text-white"
                            : point?.pointType === "tujuan"
                                ? "bg-sky-500 ring-sky-100 text-white"
                                : "bg-amber-500 ring-amber-100 text-white";

                        return (
                            <div key={`${point?.orderNumber}-${point?.pathName}-${index}`} className="group relative flex items-center gap-3">
                                {/* Dot */}
                                <div className={`relative z-10 flex h-3 w-3 shrink-0 items-center justify-center rounded-full ring-4 ${toneClass}`}>
                                ...
                                <div className="flex flex-col">
                                    <div className="flex items-center gap-1.5">
                                        <span className="text-[10px] font-black text-slate-300 group-hover:text-slate-400 transition-colors">
                                            {String(point.orderNumber).padStart(2, "0")}
                                        </span>
                                    ...
                                    {point.note && (
                                        <span className="text-[9px] text-slate-400 italic font-medium leading-none mt-0.5">{point.note}</span>
                                    )}
                                </div>
                            </div>
                        );
    ...
    const runRouteMutation = async (body, successMessage) => {
        ...
        try {
            const endpoint = routeEditMode === "ubah_jalur"
            ...
            setRouteError(requestError instanceof Error ? requestError.message : "Terjadi kesalahan saat memproses jalur.");
        } finally {
            setRouteBusy(false);
        }
    };
                ...
                setDraftRoutePoints((prev) =>
                    prev.map((p) =>
                        p.id === editingRoutePointId
                            ? { ...p, pathName: normalizedPathName, pointType: routePointForm.pointType, note: routePointForm.note }
                            : p
                    )
                );
                setEditingRoutePointId(null);
    ...
    const handleCommitDraft = async () => {
        if (!routeChangeNote.trim()) {
            setRouteError("Catatan perubahan wajib diisi untuk menyimpan struktur baru.");
            return;
        }
        ...
        try {
            const payload = {
                ...
                points: (Array.isArray(draftRoutePoints) ? draftRoutePoints : []).map((p, idx) => ({
                    pathName: p.pathName,
            ...
            const response = await fetch(`${API_BASE_URL}/api/customers/${customer.id}/routes/change`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(payload),
            });
            ...
            setRouteError(requestError instanceof Error ? requestError.message : "Gagal menyimpan jalur.");
        } finally {
            setRouteBusy(false);
        }
    };
    ...
    const handleDraftMove = (pointId, direction) => {
        setDraftRoutePoints(prev => {
            ...
            const currentIndex = transitPoints.findIndex(p => String(p.id) === String(pointId));
    ...
    const handleDraftDelete = (pointId) => {
        setDraftRoutePoints(prev => prev.filter(p => (String(p.id) !== String(pointId) || p.pointType !== 'transit')).map((p, idx) => ({ ...p, orderNumber: idx + 1 })));
    };

    const handleDeleteHistory = async (historyId) => {
        if (!confirm("Hapus catatan riwayat ini? Tindakan ini tidak dapat dibatalkan.")) return;

        setRouteBusy(true);
        setRouteError("");
        setRouteFeedback("");
        try {
            await fetchJson(`${API_BASE_URL}/api/customers/${customer.id}/routes/history/${historyId}`, {
                method: "DELETE",
            });
            setRouteFeedback("Riwayat berhasil dihapus.");
            await loadDetail();
        } catch (requestError) {
            setRouteError(requestError instanceof Error ? requestError.message : "Gagal menghapus riwayat.");
        } finally {
            setRouteBusy(false);
        }
    };
    ...
    const handleDeleteAllHistory = async () => {
        if (!confirm("Hapus seluruh riwayat jalur tenant ini? Tindakan ini tidak dapat dibatalkan.")) return;

        setRouteBusy(true);
        setRouteError("");
        setRouteFeedback("");
        try {
            const result = await fetchJson(`${API_BASE_URL}/api/customers/${customer.id}/routes/history`, {
                method: "DELETE",
            });
            ...
            setRouteError(requestError instanceof Error ? requestError.message : "Gagal menghapus semua riwayat.");
        } finally {
            setRouteBusy(false);
        }
    };
    ...
    const updateInvoiceFollowUpDraftField = (invoiceId, followUpId, value) => {
        const followUpKey = followUpId === null || followUpId === undefined ? "initial" : String(followUpId);
        setInvoiceDrafts((previousDrafts) => ({
            ...previousDrafts,
            [invoiceId]: {
                ...(previousDrafts[invoiceId] ?? {}),
                followUps: {
                    ...((previousDrafts[invoiceId] ?? {}).followUps ?? {}),
                    [followUpKey]: {
                        ...((((previousDrafts[invoiceId] ?? {}).followUps ?? {})[followUpKey]) ?? {}),
                        invoiceNumber: value,
                    },
                },
            },
    ...
    const getInvoiceFollowUpDraft = (invoice, followUp = null) => {
        ...
        return {
            invoiceNumber: String(draft.followUps?.[followUpKey]?.invoiceNumber ?? followUp?.invoiceNumber ?? ""),
        };
    };
    ...
    const getInvoiceFollowUpSourceLabel = (followUp) => followUp?.source === "auto"
        ? "Auto"
    ...
    const handleSaveInvoiceRow = async (invoice) => {
        ...
        try {
            const followUpPayload = getInvoiceFollowUps(invoice).map((followUp) => ({
                id: followUp.id,
                invoiceNumber: getInvoiceFollowUpDraft(invoice, followUp).invoiceNumber.trim() || null,
            }));
    ...
    const handleUploadInvoiceFile = async (invoice, file, followUpId = null) => {
        ...
        const draft = getInvoiceDraft(invoice);
        const targetFollowUp = getInvoiceFollowUps(invoice).find((followUp) => followUp.id === followUpId) ?? null;
        const followUpDraft = getInvoiceFollowUpDraft(invoice, targetFollowUp);
        const validationMessage = validateInvoiceDraftForUpload(draft, followUpDraft);
    ...
    const handleAddInvoiceFollowUp = async (invoice) => {
        setIsSavingInvoice(true);
        setError("");
        setInvoiceFeedback("");
        try {
            await fetchJson(`${API_BASE_URL}/api/customers/${customer.id}/invoices/${invoice.id}/follow-ups`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({}),
            });
            setInvoiceFeedback(`Split invoice manual untuk pembayaran ke-${invoice.paymentOrder} berhasil ditambahkan.`);
            await Promise.all([loadDetail(), onRefreshAll?.()]);
        } catch (requestError) {
            setError(requestError instanceof Error ? requestError.message : "Gagal menambah split invoice.");
        } finally {
            setIsSavingInvoice(false);
        }
    };
    ...
    const renderInvoiceNumberSplits = (invoice, isEditingRow) => {
        const followUps = getInvoiceFollowUps(invoice);
        const slots = followUps.length > 0 ? followUps : [null];

        return (
            <div className="flex min-w-[16rem] flex-col divide-y divide-slate-100 overflow-hidden rounded-lg border border-slate-100 bg-white">
                {slots.map((followUp, index) => {
                    const followUpDraft = getInvoiceFollowUpDraft(invoice, followUp);
                    const splitOrder = Number(followUp?.splitOrder ?? index + 1);
                    const sourceLabel = followUp ? getInvoiceFollowUpSourceLabel(followUp) : "Draft";
                    const title = followUp?.title ?? "Invoice Awal";
                    const description = followUp?.description ?? "Siapkan nomor invoice awal sebelum upload berkas invoice.";

                    return (
                        <div key={followUp?.id ?? `initial-${invoice.id}`} className="px-3 py-2">
                            <div className="mb-2 flex items-center justify-between gap-2">
                                <span className="rounded bg-slate-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-600">Split {splitOrder}</span>
                                <span className="text-[10px] font-semibold text-slate-400">{sourceLabel}</span>
                            </div>
                            <p className="text-[11px] font-semibold text-on-surface">{title}</p>
                            <p className="mt-1 text-[10px] text-slate-400">{description}</p>
                            <input
                                className={`mt-2 w-full rounded-lg border px-2 py-1.5 text-xs ${isEditingRow && !invoice?.paymentProofFileUrl ? "border-slate-200 bg-white text-on-surface" : "cursor-not-allowed border-slate-100 bg-slate-100 text-slate-500"}`}
                                disabled={!isEditingRow || Boolean(invoice?.paymentProofFileUrl) || isSavingInvoice}
                                onChange={(event) => updateInvoiceFollowUpDraftField(invoice.id, followUp?.id ?? null, event.target.value)}
                                placeholder={followUp ? "Nomor invoice split" : "Nomor invoice awal"}
                                type="text"
                                value={followUpDraft.invoiceNumber}
                            />
                        </div>
                    );
                })}
    ...
    const renderInvoiceUploadSplits = (invoice) => {
        const followUps = getInvoiceFollowUps(invoice);
        const slots = followUps.length > 0 ? followUps : [null];
        const rowDraft = getInvoiceDraft(invoice);
        const baseValidationMessage = validateInvoiceDraftBase(rowDraft);
        const paymentSettled = Boolean(invoice?.paymentProofFileUrl);

        return (
            <div className="flex min-w-[16rem] flex-col divide-y divide-slate-100 overflow-hidden rounded-lg border border-slate-100 bg-white">
                {slots.map((followUp, index) => {
                    const splitOrder = Number(followUp?.splitOrder ?? index + 1);
                    const uploadReady = !paymentSettled && !baseValidationMessage;
                    const hasFile = isOpenableFileUrl(followUp?.invoiceFileUrl);
                    const helperText = paymentSettled
                        ? "Pembayaran sudah lunas. Split baru tidak diperlukan."
                        : hasFile
                            ? "Invoice untuk split ini sudah terunggah."
                            : baseValidationMessage ?? "Upload invoice untuk split ini.";

                    return (
                        <div key={followUp?.id ?? `upload-initial-${invoice.id}`} className="px-3 py-2">
                            <div className="mb-2 flex items-center justify-between gap-2">
                                <span className="rounded bg-slate-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-600">Split {splitOrder}</span>
                                <span className="text-[10px] font-semibold text-slate-400">{followUp ? getInvoiceFollowUpSourceLabel(followUp) : "Draft"}</span>
                            </div>
                            {hasFile ? (
                                <a className="text-[11px] font-bold uppercase tracking-wider text-primary hover:underline" href={followUp.invoiceFileUrl} rel="noreferrer" target="_blank">
                                    Buka Invoice
                                </a>
                            ) : (
                                <input
                                    className="w-full text-[10px] text-on-surface-variant file:mr-2 file:rounded-md file:border-0 file:bg-primary/10 file:px-2 file:py-1 file:text-[10px] file:text-primary disabled:opacity-60"
                                    disabled={!uploadReady || isSavingInvoice}
                                    onChange={(event) => {
                                        void handleUploadInvoiceFile(invoice, event.target.files?.[0] ?? null, followUp?.id ?? null);
                                    }}
                                    type="file"
                                />
                            )}
                            <p className={`mt-1 text-[10px] font-semibold ${hasFile ? "text-emerald-600" : "text-amber-600"}`}>{helperText}</p>
                        </div>
                    );
                })}
            </div>
    ...
    const handleAddTenantRenewalSplit = async (row) => {
        if (!row?.contractId || !row?.versionId) {
            setError("Versi kontrak tenant belum tersedia untuk split tindak lanjut.");
            return;
        }

        setError("");
        try {
            await fetchJson(`${API_BASE_URL}/api/customers/${customer.id}/contracts/${row.contractId}/versions/${row.versionId}/follow-ups`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({}),
            });
            ...
            setError(requestError instanceof Error ? requestError.message : "Gagal menambah split tindak lanjut tenant.");
        }
    };
    ...
    const handleUploadTenantRenewal = async (row, file, followUpId = null) => {
        ...
        try {
            ...
            await fetchJson(`${API_BASE_URL}/api/customers/${customer.id}/contracts/${row.contractId}/versions/${row.versionId}/renewal`, {
                method: "POST",
                body: formData,
            });
            await Promise.all([loadDetail(), onRefreshAll?.()]);
        } catch (requestError) {
            setError(requestError instanceof Error ? requestError.message : "Gagal mengunggah berkas perpanjangan tenant.");
        }
    };

    const handleRespondTenantRenewal = async (row, decision, file, followUpId = null) => {
        if (!file || !row?.contractId || !row?.versionId) {
            return;
        }
        ...
        try {
            ...
            await fetchJson(`${API_BASE_URL}/api/customers/${customer.id}/contracts/${row.contractId}/versions/${row.versionId}/response`, {
                method: "POST",
                body: formData,
            });
            await Promise.all([loadDetail(), onRefreshAll?.()]);
        } catch (requestError) {
            setError(requestError instanceof Error ? requestError.message : "Gagal mengunggah tanggapan tenant.");
        }
    };
                ...
                return (
                    <label className="cursor-pointer rounded bg-slate-100 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-600 hover:bg-slate-200">
                        ...
                        <input type="file" className="hidden" onChange={(event) => {
                            void handleUploadTenantRenewal(row, event.target.files?.[0] ?? null);
                        }} />
                    </label>
                );
            }

            return <span className="rounded bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-600">Belum Ada</span>;
        }
                                ...
                                <div className="mt-2">
                                    {hasRenewalFile ? (
                                        ...
                                        <label className="cursor-pointer rounded bg-slate-100 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-600 hover:bg-slate-200">
                                            Upload
                                            <input type="file" className="hidden" onChange={(event) => {
                                                void handleUploadTenantRenewal(row, event.target.files?.[0] ?? null, followUp.id);
                                            }} />
                                        </label>
                                    )}
                                </div>
                                    ...
                                    {hasResponseFile ? (
                                        ...
                                        <div className="flex flex-col items-start gap-2">
                                            <label className="relative rounded bg-primary px-2 py-1 text-[10px] font-bold text-white">
                                                Lanjut
                                                <input type="file" className="absolute inset-0 cursor-pointer opacity-0" onChange={(event) => {
                                                    void handleRespondTenantRenewal(row, "lanjut", event.target.files?.[0] ?? null, followUp.id);
                                                }} />
                                            </label>
                                            <label className="relative rounded border border-slate-200 bg-white px-2 py-1 text-[10px] font-bold text-slate-700">
                                                Tidak
                                                <input type="file" className="absolute inset-0 cursor-pointer opacity-0" onChange={(event) => {
                                                    void handleRespondTenantRenewal(row, "tidak", event.target.files?.[0] ?? null, followUp.id);
                                                }} />
                                            </label>
                                        </div>
                                    ) : (
                    ...
                    <div className="flex flex-col justify-between gap-6 xl:flex-row xl:items-start">
                        <div>
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-3">
                                ...
                                <span className={`rounded-full px-3 py-1 text-xs font-bold ${detail?.status === "aktif" ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}>{detail?.status === "aktif" ? "Beroperasi" : "Berhenti"}</span>
                        ...
                        <div className="flex flex-wrap gap-3">
                            ...
                            <button className="rounded-xl bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-700 transition-colors hover:bg-amber-100" onClick={() => onEditTenant?.(detail ?? customer)} type="button">Edit Tenant</button>
                                                        ...
                                                        {item.code === "contract_number_missing_local" && (
                                                            <div className="mt-2 flex flex-wrap items-center gap-2">
                                                                ...
                                                                <button
                                                                    ...
                                                                    type="button"
                                                                >
                                                                    {isSavingContractNumber ? "Menyimpan..." : "Simpan Nomor"}
                                                                </button>
                                                                <button
                                                                    ...
                                                                    onClick={() => {
                                                                        if (primaryContractRowMarkerId) {
                                                                            toggleContractNumberEmptyMark(primaryContractRowMarkerId);
                                                                        }
                                                                    }}
                                                                    type="button"
                                                                >
                                            ...
                                            {requiredDocuments.slice(0, 3).map((doc) => (
                                                <div key={doc.id} className="flex items-center gap-3 rounded-lg border border-slate-100 bg-white px-4 py-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-on-surface-variant">{doc.nomorDokumen || "-"} â€¢ {formatDate(doc.tanggalDokumen)}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                ...
                {activeTab === "jalur" && (
                    <div className="space-y-6">
                        ...
                        <section className="rounded-2xl bg-surface-container-lowest p-6 shadow-sm border-l-4 border-amber-400">
                            <div className="flex flex-wrap items-start justify-between gap-4">
                                <div className="max-w-2xl">
                                    <h2 className="text-xl font-bold text-on-surface">Manajemen Jalur Lintasan</h2>
                                    <p className="mt-1 text-sm text-on-surface-variant">
                                        Kelola lintasan menuju lokasi tenant. Pisahkan antara penambahan titik cepat atau perubahan struktur jalur yang dicatat ke riwayat.
                                    </p>
                                </div>
                                    ...
                                    {!isRouteDrafting ? (
                                        ...
                                        <div className="flex items-center gap-3 animate-in fade-in slide-in-from-right-4 duration-300">
                                            <div className="flex flex-col items-end mr-2">
                                                <span className="text-[10px] font-black uppercase tracking-widest text-amber-600 animate-pulse">Sedang Menyunting Draft</span>
                                                <span className="text-[9px] text-slate-500 italic">Jalur lama tetap aktif sampai disimpan</span>
                                            </div>
                                            <button
                            ...
                            {isRouteDrafting && (
                                <div className="mt-4 rounded-xl border-2 border-amber-400 bg-amber-50/80 p-5 animate-in zoom-in-95 duration-300 shadow-inner">
                                    <div className="flex items-center gap-3 text-amber-900 border-b border-amber-200 pb-3 mb-4">
                                        ...
                                        <div>
                                            <p className="text-base font-black">Mode Penyuntingan Struktur Jalur</p>
                                            <p className="text-xs text-amber-700 font-medium italic">Menyimpan akan membuat riwayat (versi) baru dan secara otomatis mengaktifkan struktur ini sebagai jalur utama tenant.</p>
                                        </div>
                                    </div>

                                    <div className="ml-0 max-w-2xl">
                                        <label className="mb-1.5 block text-[11px] font-black uppercase tracking-widest text-amber-800">Alasan Perubahan / Catatan Riwayat (Wajib)</label>
                                        <div className="flex gap-2">
                                            <span className="material-symbols-outlined text-amber-400 mt-1.5">draw</span>
                                            <input
                        ...
                        <div className="flex flex-col gap-8">
                            <div className="space-y-8">
                                {/* Glosarium Jalur - Moved Above Form */}
                                <section className="rounded-2xl bg-indigo-900 p-6 text-white shadow-xl shadow-indigo-100 relative overflow-hidden">
                                    ...
                                    <div className="flex flex-wrap items-center justify-between gap-4 relative z-10">
                                        ...
                                        <div className="flex flex-wrap gap-3">
                                            <div className="flex items-center gap-2 rounded-lg bg-white/10 px-3 py-1.5 backdrop-blur-sm">
                                                ...
                                                <span className="text-[10px] font-black uppercase tracking-widest text-indigo-200">Awal:</span>
                                                <span className="text-xs text-white/90">Manhole Utama/ODP</span>
                                            </div>
                                            <div className="flex items-center gap-2 rounded-lg bg-white/10 px-3 py-1.5 backdrop-blur-sm">
                                                <span className="h-2 w-2 rounded-full bg-slate-400"></span>
                                                <span className="text-[10px] font-black uppercase tracking-widest text-indigo-200">Transit:</span>
                                                <span className="text-xs text-white/90">Tiang/Lintasan</span>
                                            </div>
                                            <div className="flex items-center gap-2 rounded-lg bg-white/10 px-3 py-1.5 backdrop-blur-sm">
                                                <span className="h-2 w-2 rounded-full bg-emerald-400"></span>
                                                <span className="text-[10px] font-black uppercase tracking-widest text-indigo-200">Tujuan:</span>
                                                <span className="text-xs text-white/90">Lokasi Tenant</span>
                                            </div>
                                        </div>
                                    </div>
                                </section>
                                ...
                                <section id="route-point-form" className="rounded-2xl bg-surface-container-lowest p-6 shadow-sm">
                                    <h3 className="mb-4 text-sm font-bold text-on-surface flex items-center gap-2">
                                        ...
                                        {editingRoutePointId ? "Update Titik Jalur" : "Tambah Titik Baru"}
                                    </h3>
                                    <form className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4" onSubmit={(event) => { void handleSubmitRoutePoint(event); }}>
                                        <div className="md:col-span-2">
                                            <label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-on-surface-variant">Nama Jalan / Lintasan</label>
                                            <input
                                                ...
                                                onChange={(event) => setRoutePointForm((previous) => ({ ...previous, pathName: event.target.value }))}
                                        ...
                                        <div className="md:col-span-2">
                                            <label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-on-surface-variant">Jenis Titik</label>
                                            <div className="grid grid-cols-3 gap-2 rounded-lg border border-slate-200 bg-slate-50 p-1">
                                                {Object.entries(routePointTypeMeta).map(([typeKey, meta]) => {
                                                    const isDisabled =
                                                        !activeRouteId
                                                        || (typeKey === "awal" && Boolean(activeAwalPoint && activeAwalPoint.id !== editingRoutePointId))
                                                        || (typeKey === "tujuan" && Boolean(activeTujuanPoint && activeTujuanPoint.id !== editingRoutePointId));

                                                    const isActive = routePointForm.pointType === typeKey;
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            disabled={isDisabled}
                                                            onClick={() => setRoutePointForm((previous) => ({ ...previous, pointType: typeKey }))}
                                                            type="button"
                                                        >
                                        ...
                                        <div className="md:col-span-3">
                                            <label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-on-surface-variant">Keterangan Teknis</label>
                                            <input
                                                className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-primary outline-none"
                                                disabled={!activeRouteId}
                                                onChange={(event) => setRoutePointForm((previous) => ({ ...previous, note: event.target.value }))}
                                    ...
                                    {(routeError || routeFeedback) && (
                                        <div className={`mt-3 rounded-lg border px-3 py-2 text-xs font-semibold animate-in slide-in-from-top-2 duration-300 ${routeError ? "border-rose-200 bg-rose-50 text-rose-700" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>
                                            {routeError || routeFeedback}
                                        </div>
                                    )}
                                </section>
                                ...
                                <section className="rounded-2xl bg-surface-container-lowest p-6 shadow-sm overflow-hidden">
                                    <h3 className="mb-4 text-sm font-bold text-on-surface flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <label className="text-[10px] uppercase font-black text-on-surface-variant">Status: </label>
                                            <select
                                                className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-bold"
                                                disabled={routeBusy || !activeRouteId}
                                                onChange={(event) => handleFlowStatusChange(event.target.value)}
                                                value={isRouteDrafting ? draftRouteStatus : activeRouteStatus}
                                            >
                                        ...
                                        <table className="w-full border-collapse">
                                            <thead>
                                                <tr className="border-b border-slate-100 bg-slate-50/50">
                                                    ...
                                                    <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Lintasan</th>
                                                    <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Tipe</th>
                                                    <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Note</th>
                                            ...
                                            <tbody className="divide-y divide-slate-50">
                                                {(isRouteDrafting ? draftRoutePoints : routePoints).map((point) => (
                                                    <tr key={point.id} className="bg-white hover:bg-slate-50/50 transition-colors">
                                                        <td className="px-4 py-4 text-sm font-bold text-slate-400">{point.orderNumber}</td>
                                                        <td className="px-4 py-4 text-sm font-semibold text-on-surface">{point.pathName}</td>
                                                        <td className="px-4 py-4">
                                                            <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-black uppercase tracking-wider border ${point.pointType === 'awal' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' :
                                                                    point.pointType === 'tujuan' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                                                        'bg-slate-50 text-slate-600 border-slate-100'
                                                                }`}>
                                                                <span className="material-symbols-outlined text-[13px]">{routePointTypeMeta[point.pointType]?.icon}</span>
                                                                {routePointTypeLabelMap[point.pointType]}
                                                            </span>
                                                        </td>
                                                        <td className="px-4 py-4 text-[11px] text-on-surface-variant italic">{point.note || "-"}</td>
                                                ...
                                                {(isRouteDrafting ? draftRoutePoints : routePoints).length === 0 && (
                                                    <tr>
                                                        <td className="px-4 py-10 text-center text-sm text-on-surface-variant italic" colSpan="5">
                            ...
                            <div className="relative py-4">
                                ...
                                <div className="relative flex justify-center">
                                    <span className="bg-slate-50 px-4 text-xs font-black uppercase tracking-[0.3em] text-slate-400">Arsip & Riwayat Perubahan</span>
                                </div>
                            </div>
                                    ...
                                    <div className="mb-4 flex items-start justify-between gap-3">
                                        <div>
                                            ...
                                            <p className="mt-1 text-xs text-on-surface-variant">
                                                Setiap sesi <span className="font-bold text-on-surface">Ubah Jalur</span> disimpan sebagai jejak perubahan, jadi bukan hanya kondisi terbaru yang tampil.
                                            </p>
                                    ...
                                    <div className="space-y-8 overflow-auto max-h-[900px] pr-2 custom-scrollbar">
                                        {routeHistory.length === 0 ? (
                                            <div className="rounded-3xl border-2 border-dashed border-slate-100 bg-slate-50/50 px-4 py-12 text-center">
                                                ...
                                                <p className="text-xs font-bold text-slate-400">Belum ada catatan riwayat jalur.</p>
                                            </div>
                                        ) : (
                                                    ...
                                                    <div key={item.id} className="group relative">
                                                        ...
                                                        >
                                                            <div className="space-y-1">
                                                                <div className="flex items-center gap-2">
                                                                    <span className="material-symbols-outlined text-slate-400 text-sm transition-transform duration-300" style={{ transform: isExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                                                                        expand_more
                                                                    </span>
                                                                    ...
                                                                    <span className="text-xs font-black text-on-surface">{formatDateTime(item.createdAt)}</span>
                                                                </div>
                                                                ...
                                                                <div className="relative mt-2 max-w-2xl overflow-hidden rounded-2xl border border-amber-100 bg-gradient-to-r from-amber-50/50 to-transparent p-3 pl-4 hover:border-amber-300 transition-colors">
                                                                    ...
                                                                    <div className="flex items-start gap-2">
                                                                        <span className="material-symbols-outlined text-sm text-amber-500 mt-0.5">format_quote</span>
                                                                        <p className="text-xs font-bold italic leading-relaxed text-amber-900">
                                                                            {item.note || "Perubahan struktur jalur tanpa catatan tambahan."}
                                                                        </p>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                                ...
                                                                <table className="w-full border-collapse">
                                                                    <thead>
                                                                        <tr className="border-b border-slate-50 bg-slate-50/30">
                                                                            <th className="px-4 py-2.5 text-left text-[9px] font-black uppercase tracking-wider text-slate-400">#</th>
                                                                            <th className="px-4 py-2.5 text-left text-[9px] font-black uppercase tracking-wider text-slate-400">Lintasan</th>
                                                                            <th className="px-4 py-2.5 text-left text-[9px] font-black uppercase tracking-wider text-slate-400">Tipe</th>
                                                                            <th className="px-4 py-2.5 text-left text-[9px] font-black uppercase tracking-wider text-slate-400">Note</th>
                                                                        </tr>
                                                                    </thead>
                                                                    <tbody className="divide-y divide-slate-50">
                                                                        {item.afterPoints.map((point) => (
                                                                    <tr key={point.id} className="bg-white hover:bg-slate-50/30 transition-colors">
                                                                        <td className="px-4 py-3 text-[11px] font-bold text-slate-400">{point.orderNumber}</td>
                                                                        <td className="px-4 py-3 text-[11px] font-bold text-on-surface">{point.pathName}</td>
                                                                        <td className="px-4 py-3">
                                                                            <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wider border ${point.pointType === 'awal' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' :
                                                                                    point.pointType === 'tujuan' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                                                                        'bg-slate-50 text-slate-600 border-slate-100'
                                                                                }`}>
                                                                                <span className="material-symbols-outlined text-[12px]">{routePointTypeMeta[point.pointType]?.icon}</span>
                                                                                {routePointTypeLabelMap[point.pointType]}
                                                                            </span>
                                                                        </td>
                                                                        <td className="px-4 py-3 text-[10px] text-on-surface-variant italic font-medium">{point.note || "-"}</td>
                                                                    </tr>
                                                                ))}
                                                                {item.afterPoints.length === 0 && (
                                                                    <tr>
                                                                        <td className="px-4 py-6 text-center text-[10px] text-slate-400 italic" colSpan="4">
                            ...
                            <table className="w-full border-collapse">
                                <thead>
                                    <tr className="border-b border-slate-100 bg-slate-50/50">
                                        ...
                                        <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Keterangan</th>
                                        <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Periode Berjalan (Awal)</th>
                                        <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Periode Berjalan (Akhir)</th>
                                        <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Paket</th>
                                        <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Jumlah Paket</th>
                                ...
                                <tbody>
                                    {contractRowsForTable.map((row) => {
                                        ...
                                        return (
                                            <tr key={row.id} className="bg-white border-b border-slate-50">
                                                <td className="px-4 py-4 text-sm font-semibold text-on-surface">{row.number}</td>
                                                <td className="px-4 py-4 text-sm text-on-surface-variant">
                                                    {row.contractNumber ? (
                                                        <span className="font-semibold text-on-surface">{row.contractNumber}</span>
                                                    ) : (
                                                        <div className="flex flex-col items-start gap-1">
                                                            {!isContractNumberMarkedEmpty && (
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        type="button"
                                                                    >
                                                                        {isSavingContractNumber ? "Simpan..." : "Simpan"}
                                                                    </button>
                                                                </div>
                                                            )}
                                                            <span className={`rounded px-2 py-1 text-[10px] font-bold ${isContractNumberMarkedEmpty ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>
                                                            ...
                                                            <button
                                                                ...
                                                                type="button"
                                                            >
                                                                {isContractNumberMarkedEmpty ? "Batal Tandai" : "Tandai Memang Kosong"}
                                                            </button>
                                                        </div>
                                                    )}
                                                </td>
                                                <td className="px-4 py-4 text-sm text-on-surface-variant">
                                                    <span className="rounded bg-blue-50 px-2 py-1 text-[10px] font-bold text-blue-700">{row.note}</span>
                                                </td>
                                                <td className="px-4 py-4 text-sm whitespace-nowrap text-on-surface-variant">{formatDate(row.periodStart)}</td>
                                                <td className="px-4 py-4 text-sm whitespace-nowrap text-on-surface-variant">{formatDate(row.periodEnd)}</td>
                                                ...
                                                <td className="px-4 py-4 text-sm text-on-surface-variant">
                                                    {row.hasBak ? (
                                                        ...
                                                        <div className="flex flex-col items-start gap-1">
                                                            ...
                                                            <span className={`rounded px-2 py-1 text-[10px] font-bold ${isBakMarkedEmpty ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>
                                                                {isBakMarkedEmpty ? "Memang Kosong" : "Belum Diunggah"}
                                                            </span>
                                                            <button
                                                                ...
                                                                type="button"
                                                            >
                                                                {isBakMarkedEmpty ? "Batal Tandai" : "Tandai Memang Kosong"}
                                                            </button>
                                                        </div>
                                                    )}
                                                </td>
                                                <td className="px-4 py-4 text-sm text-on-surface-variant">
                                                    <div className="space-y-2">
                                                        ...
                                                        <button
                                                            ...
                                                            title={!hasInitialTenantRenewalUpload(row) ? "Upload berkas perpanjangan pertama terlebih dahulu." : "Tambah split tindak lanjut tenant"}
                                                            type="button"
                                                        >
                                                            Tambah Split
                                                        </button>
                                                ...
                                                <td className="px-4 py-4 text-right text-sm">
                                                    <button className="text-amber-600 font-bold hover:underline" type="button">Edit</button>
                                                </td>
                                            </tr>
                                        );
                                    })}
                                    {contractRowsForTable.length === 0 && (
                                        <tr>
                                            <td className="px-4 py-6 text-center text-sm text-on-surface-variant" colSpan="11">
                    ...
                    <section className="rounded-2xl bg-surface-container-lowest p-6 shadow-sm">
                        ...
                        <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-2">
                            <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-700">
                                Jadwal invoice aktif di bawah ini selalu mengikuti periode tagihan terbaru.
                            </div>
                            <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
                                Riwayat invoice dipisahkan agar perubahan restrukturisasi mudah dipantau.
                            </div>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                            <div className="mb-3 flex items-center justify-between gap-3">
                                <div>
                                    <h3 className="text-sm font-bold text-on-surface">Jadwal Invoice Aktif</h3>
                                    <p className="text-xs text-on-surface-variant">Hanya sisa tagihan aktif yang ditampilkan dan bisa diedit di sini.</p>
                                </div>
                                <div className="rounded-full bg-blue-100 px-3 py-1 text-[11px] font-bold text-blue-700">{invoiceRows.length} invoice aktif</div>
                            </div>
                                                ...
                                                <td className="px-4 py-4 text-sm">
                                                    <div className="flex flex-col items-start gap-1">
                                                        ...
                                                        {isOpenableFileUrl(invoice.invoiceFileUrl) && (
                                                            <a className="text-[10px] font-bold text-primary hover:underline" href={invoice.invoiceFileUrl} rel="noreferrer" target="_blank">
                                                                Buka invoice
                                                            </a>
                                                        )}
                                                    </div>
                                                ...
                                                <td className="px-4 py-4 text-sm">
                                                    <div className="flex flex-col items-start gap-1">
                                                        ...
                                                        {isOpenableFileUrl(invoice.paymentProofFileUrl) && (
                                                            <a className="text-[10px] font-bold text-primary hover:underline" href={invoice.paymentProofFileUrl} rel="noreferrer" target="_blank">
                                                                Buka bukti bayar
                                                            </a>
                                                        )}
                                                    </div>
                        ...
                        <div className="mt-8 overflow-x-auto">
                            <div className="mb-3 flex items-center justify-between gap-3">
                                <div>
                                    <h3 className="text-sm font-bold text-on-surface">Riwayat Invoice</h3>
                                    <p className="text-xs text-on-surface-variant">Invoice lunas dan invoice dari jadwal lama tersimpan di bagian ini.</p>
                                </div>
                                <div className="rounded-full bg-slate-200 px-3 py-1 text-[11px] font-bold text-slate-700">{historyInvoiceRows.length} invoice riwayat</div>
                            </div>
                            <table className="w-full min-w-[1200px] border-collapse">
                                <thead>
                                    <tr className="border-b border-slate-100 bg-slate-50/50">
                                        ...
                                        <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Pembayaran Ke</th>
                                        <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Nomor Invoice</th>
                                        <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Periode</th>
                                        <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Jumlah</th>
                                        <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Status</th>
                                        <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Terbayar</th>
                                        <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wider text-on-surface-variant">Versi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    {displayHistoryInvoiceRows.map((invoice, idx) => {
                                        ...
                                        return (
                                            <tr key={invoice.id} className="border-b border-slate-50 bg-white">
                                                <td className="px-4 py-4 text-sm font-medium text-on-surface">{idx + 1}</td>
                                                <td className="px-4 py-4 text-sm font-semibold text-on-surface">Pembayaran ke-{invoice.paymentOrder}</td>
                                                <td className="px-4 py-4 text-sm text-on-surface">{invoice.invoiceNumber || "-"}</td>
                                                <td className="px-4 py-4 text-sm text-on-surface-variant">{periodLabel}</td>
                                                <td className="px-4 py-4 text-sm font-semibold text-on-surface">{formatCurrency(invoice.amount ?? 0)}</td>
                                                <td className="px-4 py-4 text-sm">
                                                    <span className={`rounded px-2 py-1 text-[10px] font-bold uppercase tracking-widest ${statusMeta.badgeClass}`}>{statusMeta.label}</span>
                                                </td>
                                                <td className="px-4 py-4 text-sm text-on-surface-variant">{formatDate(invoice.paidAt)}</td>
                                                <td className="px-4 py-4 text-sm">
                    ...
                    <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.2fr_0.8fr]">
                        <section className="rounded-2xl bg-surface-container-lowest p-6 shadow-sm">
                            ...
                            <div className="space-y-3">
                                {allDocuments.map((document) => (
                                    <div key={document?.id} className="flex items-center justify-between rounded-xl border border-slate-100 bg-white px-4 py-3">
                                        <div>
                                            <p className="text-sm font-semibold text-on-surface">{documentTypeLabelMap[document?.jenisDokumen] || document?.jenisDokumen}</p>
                                            <p className="mt-1 text-xs text-on-surface-variant">{document?.nomorDokumen || "-"} • {formatDate(document?.tanggalDokumen)}</p>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            {isOpenableFileUrl(document?.fileUrl) && (
                                                <a className="text-xs font-bold text-primary hover:underline" href={document.fileUrl} rel="noreferrer" target="_blank">
                                                    Buka berkas
                                                </a>
                                            )}
                                            <button className="text-xs font-bold text-amber-600 hover:underline">Edit</button>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <section className="rounded-2xl bg-surface-container-lowest p-6 shadow-sm">
                            ...
                            <form className="space-y-4" onSubmit={handleUploadDocument}>
                                ...
                                <div>
                                    <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-on-surface-variant">Upload Dokumen</label>
                                    <input
                                    ...
                                    <p className="mt-2 text-xs text-on-surface-variant">
                                        {documentDraft.uploadedFileName ? `File dipilih: ${documentDraft.uploadedFileName}` : "Belum ada file dipilih."}
                                    </p>
                                </div>
                                ...
                                <p className="text-xs text-on-surface-variant">Jika tanggal dokumen dikosongkan, sistem akan otomatis menyimpan tanggal hari ini.</p>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/20 px-4">
                        <div className="w-full max-w-md rounded-2xl border border-slate-100 bg-white p-6 shadow-2xl">
                            <div className="mb-4 flex items-start justify-between gap-4">
                                <div>
                                    <p className="text-xs font-black uppercase tracking-widest text-primary">Invoice</p>
                                    <h3 className="text-xl font-bold text-on-surface">Edit Periode Tagihan</h3>
                                    <p className="text-xs text-on-surface-variant">Ubah billing cycle kontrak langsung dari tab Invoice.</p>
                                </div>
                                <button className="rounded-lg bg-slate-100 p-2 text-on-surface-variant transition-colors hover:bg-slate-200" onClick={() => setBillingEditor(null)} type="button"><span className="material-symbols-outlined text-base">close</span></button>
                            </div>
                            <form className="space-y-4" onSubmit={handleSaveBillingCycle}>
                                <FieldInput label="Periode Tagihan" type="number" value={billingEditor.billingEvery} onChange={(value) => setBillingEditor((previous) => previous ? { ...previous, billingEvery: value } : previous)} />
                                <FieldSelect label="Satuan" value={billingEditor.billingUnit} onChange={(value) => setBillingEditor((previous) => previous ? { ...previous, billingUnit: value } : previous)} options={[{ value: "hari", label: "Hari" }, { value: "bulan", label: "Bulan" }, { value: "tahun", label: "Tahun" }]} />
                                {billingError && <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700">{billingError}</div>}
                                <div className="flex justify-end gap-2">
                                    <button className="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-on-surface-variant transition-colors hover:bg-slate-50" onClick={() => setBillingEditor(null)} type="button">Batal</button>
                                    <button className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60" disabled={isSavingBilling} type="submit">{isSavingBilling ? "Menyimpan..." : "Simpan"}</button>
                                </div>
                            </form>
                        </div>
                    </div>
                        ...
                        <div className="w-full max-w-lg rounded-2xl border border-slate-100 bg-white p-6 shadow-2xl">
                            ...
                            <form className="space-y-4" onSubmit={handleCreateVersion}>
                                ...
                                {(versionEditor.reason ?? "ubah_paket") === "lainnya" && (
                                    <FieldInput label="Input Alasan Lain" value={versionEditor.customReason ?? ""} onChange={(value) => setVersionEditor((previous) => previous ? { ...previous, customReason: value } : previous)} placeholder="Tulis alasan perubahan kontrak" />
                                )}
